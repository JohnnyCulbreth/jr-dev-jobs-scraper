...
async function main() {
  // Load indeed-dev-austin-master.json
  let masterData;
  try {
    masterData = JSON.parse(
      fs.readFileSync('indeed-dev-austin-master.json', 'utf8')
    );
  } catch (error) {
  ...
  // Save the updated masterData to indeed-dev-austin-master.json
  fs.writeFileSync('indeed-dev-austin-master.json', JSON.stringify(masterData));

  console.log('Finished scraping. Data saved to indeed-dev-austin-master.json');
